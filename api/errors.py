...
def err_dict(err, description):
    return {err.status: openapi.Response(description, examples={'application/json': err.get_errors_context()})}
...
err_401_unauthorized = ErrorMessage(status=status.HTTP_401_UNAUTHORIZED, key='detail', message=NotAuthenticated.default_detail)

err_dict_404_not_found = err_dict(err_404_not_found, 'Объект не найден')
err_dict_401_unauthorized = err_dict(err_401_unauthorized, 'Пользователь не авторизован')
